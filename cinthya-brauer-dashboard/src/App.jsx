...
const CAMPOS_LABEL = Object.assign({}, CAMPOS_PERF, CAMPOS_FREQ);
...
async function fsGet() {
  ...
  return j.fields && j.fields.data && j.fields.data.stringValue
    ? JSON.parse(j.fields.data.stringValue)
    : null;
}
...
const CustomTooltip = function({ active, payload, label }) {
  ...
  return React.createElement("div", {
    ...
    React.createElement("p", { style: { color:"#fff", margin:0, fontWeight:700, fontSize:12 } }, label),
...
function GraficoLinha({ dados, campos, titulo }) {
  const temDados = dados.some(function(d) { return campos.some(function(c) { return d[c.key] != null; }); });
  const manyPts  = dados.length > 4;
  const tickProps = manyPts
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#111", zIndex:2000, overflowY:"auto" }}>
      <div style={{ position:"sticky", top:0, zIndex:10, background:"#0d0d0d", borderBottom:"2px solid " + ORANGE, padding:"10px 16px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ fontWeight:900, fontSize:16, color:"#fff", letterSpacing:1 }}>CINTHYA BRAUER <span style={{ fontSize:10, color:GRAY, fontWeight:400, letterSpacing:2 }}>PERSONAL TRAINER</span></div>
      ...
      <div style={{ maxWidth:430, margin:"0 auto", padding:"16px 16px 40px" }}>
        <div style={{ background:CARD, borderRadius:14, padding:"18px 20px", border:"1px solid " + BORDER, marginBottom:14 }}>
          ...
          <div style={{ display:"flex", gap:20, flexWrap:"wrap" }}>
            ...
            {ultimoDado && <div><span style={{ color:GRAY, fontSize:11 }}>Periodo </span><span style={{ color:ORANGE, fontWeight:700 }}>{periodoLabel(ultimoDado)}</span></div>}
          </div>
        </div>
        {ultimoDado && (
...
function ModalDados({ tipo, atletas, onImportar, onFechar }) {
  var textoInicial = tipo === "exportar" ? JSON.stringify(atletas, null, 2) : "";
  ...
  function copiar() {
    ...
    catch(e) { setMsg("Selecione e copie manualmente."); }
  }
  function tentarImportar() {
    try { var p = JSON.parse(texto); if (!Array.isArray(p)) throw new Error(); setConfirmando(true); }
    catch(e) { setMsg("JSON invalido."); }
  }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:3000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:"#1a1a1a", borderRadius:16, padding:28, maxWidth:540, width:"100%", border:"1px solid " + BORDER }}>
        ...
        <div style={{ color:GRAY, fontSize:13, marginBottom:14 }}>{tipo === "exportar" ? "Copie o conteudo abaixo e envie:" : "Cole o JSON recebido e clique em Importar:"}</div>
        <textarea
          value={texto}
          onChange={function(e) { setTexto(e.target.value); }}
          readOnly={tipo === "exportar"}
          rows={12}
...
function GerenciarAtletas({ atletas, onArquivar, onDesarquivar, onFechar }) {
  ...
  function estilo(bg, color, border) {
          ...
          {ativos.map(function(a) {
            return (
              <div key={a.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 12px", background:"#0d0d0d", borderRadius:8, marginBottom:6, border:"1px solid " + BORDER }}>
                ...
                <button onClick={function() { onArquivar(a.id); }} style={estilo("#7f1d1d","#fca5a5","#dc2626")}>Arquivar</button>
              </div>
            );
          })}
        </div>
              ...
              {arquivados.map(function(a) {
                return (
                  <div key={a.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 12px", background:"#0d0d0d", borderRadius:8, marginBottom:6, border:"1px solid #333", opacity:0.7 }}>
                    ...
                    <button onClick={function() { onDesarquivar(a.id); }} style={estilo("#1a3a1a","#4ade80","#16a34a")}>Reativar</button>
                  </div>
                );
              })}
            </div>
...
function CardCMJ({ atleta, dadosOrdenados, onFechar }) {
  ...
  var salvarFirebase = useCallback(function(dados) {
    ...
    fsSet(dados)
      ...
      .catch(function(e) {
        console.error(e);
        showMsg("Erro ao salvar. Verifique a conexao.");
        setSaving(false);
      });
  }, []);
  ...
  var ultimosPorCampo = useMemo(function() {
    var result = {};
    var campos = Object.keys(CAMPOS_LABEL);
  ...
  function salvarAtleta() {
    ...
    setSelId(id);
    setNovoAtleta({ nome:"", idade:"", esporte:"", frequencia:"" });
    setPendente(true);
    showMsg("Atleta adicionado! Clique em Salvar.");
    setView("dashboard");
  }
  ...
  function adicionarDado() {
    ...
    var merged;
    if (ex) {
      merged = Object.assign({}, ex);
      Object.entries(d).forEach(function(entry) { if (entry[1] != null) merged[entry[0]] = entry[1]; });
    } else {
      merged = d;
    }
    var temValor = Object.entries(merged).some(function(entry) {
      return entry[0] !== "mes" && entry[0] !== "ano" && entry[0] !== "quinzena" && entry[1] != null;
    });
    if (!temValor) { showMsg("Preencha ao menos um campo."); return; }
    setAtletas(function(prev) {
      return prev.map(function(a) {
        if (a.id !== selId) return a;
        var novosDados = a.dados.filter(function(x) { return !(x.mes === d.mes && x.ano === d.ano && x.quinzena === d.quinzena); });
        novosDados.push(merged);
        return Object.assign({}, a, { dados: novosDados });
      });
    });
  ...
  function arquivarAtleta(id) {
    ...
    if (selId === id) {
      var prox = atletasAtivos.find(function(a) { return a.id !== id; });
      setSelId(prox ? prox.id : 1);
    }
    setPendente(true);
    ...
    <div style={{ minHeight:"100vh", background:BG, color:"#fff", fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:"#0d0d0d", borderBottom:"1px solid " + BORDER, padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <button
            onClick={function() { if (atletas) salvarFirebase(atletas); }}
            disabled={saving || !pendente}
      ...
      {view === "dashboard" && atleta && (
        <div style={{ padding:"20px 24px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20, flexWrap:"wrap" }}>
            ...
            <select
              style={{ background:"#1a1a1a", border:"1px solid " + BORDER, borderRadius:6, color:ORANGE, padding:"6px 12px", fontSize:15, fontWeight:700, outline:"none" }}
              value={selId}
              onChange={function(e) { setSelId(Number(e.target.value)); }}
            >
              {atletasAtivos.map(function(a) { return <option key={a.id} value={a.id}>{a.nome}</option>; })}
            </select>
            <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
              ...
              <button
                onClick={function() { setVerRelatorio(true); }}
                disabled={!ultimoDado}
